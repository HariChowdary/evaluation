...
public class Scrutiny_MarksSummryActivity extends Activity implements
		OnClickListener {
...
int TotalCount=0;
		...
		myAlertDialog.setPositiveButton("Yes",
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int arg1) {
						...
						upDateMarks();
						updateDBCompleted();
						Intent intent=new Intent(Scrutiny_MarksSummryActivity.this, ShowBundleCompletedMessage.class);
	...
	private void upDateMarks() {
		try {
			for (int i = 0; i < snArray.size(); i++) {
				String query2 = "update " + SSConstants.TABLE_EVALUATION_SAVE + " set  " + SSConstants.GRAND_TOTAL_MARK
						+ "= " + tmArray.get(i) + ", remark_grant_total='"+rmArray.get(i)+"', "+
						SSConstants.IS_SCRUTINIZED+"=1 where bundle_no ='"
						+ bundleNo + "' and " + SSConstants.ANS_BOOK_BARCODE + "= '" + snArray.get(i) + "'";
				database.executeSQLQuery(query2, null).close();
			}
	...
	private void updateDBCompleted(){
		try{
		String query5 ="update table_user set active_status=11 where TRIM(UPPER(login)) =TRIM(UPPER('"+
				userId + "'))";
		Log.v("query submit", query5);
		database.executeSQLQuery(query5, null).close();
		...
		String query2 = "update "+SSConstants.TABLE_EVALUATION_SAVE+" set scrutinize_status = 4 where scrutinize_status in (0) and bundle_no ='"
				+ bundleNo + "'";
		database.executeSQLQuery(query1, null).close();
		database.executeSQLQuery(query2, null).close();
		...
		database.executeSQLQuery(query3, null).close();
		database.executeSQLQuery(query4, null).close();
		}catch (Exception e){
			e.printStackTrace();
		}
	}
